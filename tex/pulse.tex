...
The interface exposes three user-implemented functions: (1) \code{init()} initializes the start pointer based on the data structure, (2) \code{next()} updates the current pointer, and (3) \code{end()} determines if the traversal should stop. \pulse then iteratively executes the traversal using these functions through \code{execute()}. We also introduce two key aspects of our iterator abstraction to enhance and constrain the expressiveness of operations on linked data structures.
...
\paragraphb{Stateful traversals} Pointer traversals in data structures are often stateful, with varying state types. For example, hash table lookups use the search key as state, while B-Tree summations require a running total that updates with each value. To support this, \pulse iterators include a \code{scratch\_pad} for storing arbitrary state. The state is initialized in \code{init()}, updated in \code{next()}, and finalized in \code{end()}. The \code{execute()} function returns the contents of \code{scratch\_pad} (Line~\ref{line:scratch_return}), allowing developers to access desired data.

\paragraphb{Bounded computations} \pulse accelerators enable lightweight processing for memory-intensive operations, maximizing bandwidth utilization. While \code{init()} runs on the CPU, \code{next()} and \code{end()} are offloaded to \pulse accelerators, which impose two limitations on memory accesses and computations. First, \pulse disallows nondeterministic executions, such as unbounded loops that cannot be unrolled. Second, \code{execute()} in Listing~\ref{lst:iterator} restricts the maximum iterations per request to prevent long traversals from blocking others. If the limit is exceeded, \pulse terminates the traversal and returns the \code{scratch\_pad} value to the CPU, allowing a new request to continue from that point.
  

\paragraphb{An illustrative example} We illustrate how the \code{find()} operation of the C++ STL \code{unordered\_map} can be adapted for \pulse. Listing~\ref{lst:stl} presents a simplified STL implementation, where the traversal starts by computing a hash function to find the corresponding hash bucket pointer. It then iterates through the linked list in the bucket, terminating when the key is found or the list ends.

Listing~\ref{lst:stl_mod} shows the iterator implementation in \pulse. The core logic remains largely unchanged, with minor adjustments to the \code{init()}, \code{next()}, and \code{end()} functions. The main differences lie in how the state (search key) is passed between these functions and how results are returned via the \code{scratch\_pad} (an error message if the key is not found, or its value if it is).
...
\paragraphb{Shortcomings of Prior Approaches}
