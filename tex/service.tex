...
\begin{figure}[t]
  ...
  \subfigure[\scriptsize Intermediate data (normalized by mean usage)] {
    \includegraphics[width = 1.5in]{fig/jiffy/ephemeral_avg}
    \label{fig:ephemeral-avg}
  }
  \subfigure[\scriptsize Cumulative intermediate data (normalized by peak usage)] {
    \includegraphics[width = 1.5in]{fig/jiffy/ephemeral}
...
We have realized an end-to-end implementation of \jiffy, which we will open-source along with the paper. \jiffy's data plane enables compute tasks to read/write intermediate data to their blocks via an intuitive, programmable, API (\S\ref{ssec:api}). We demonstrate the expressiveness of \jiffy's API by realizing serverless incarnations of several powerful distributed programming frameworks atop \jiffy (\S\ref{sec:models}): MapReduce~\cite{mapreduce}, Dryad~\cite{dryad}, StreamScope~\cite{streamscope} and Piccolo~\cite{piccolo}. We compare \jiffy against five state-of-the-art ephemeral storage systems over a variety of workloads. Our evaluation suggests that fine-grained resource allocation in \jiffy allows it to improve resource utilization by as much as $3\times$ and distributed analytics performance by a factor of $1.6-2.5\times$ compared to peak provisioning. 
...
\begin{figure}
  ...
  \begin{tikzpicture}[font=\scriptsize]
    \node[draw, fill=gray!30, minimum height=3em, align=center, text width=3em] (wrapper-1) {Block\#1};
    %\node[draw, fill=cyan!30, minimum height=4.5em, yshift=0.6em, right=0em of wrapper-1] (em-1) {\rotatebox{90}{Eviction Manager}};
    \node[draw, fill=gray!30, minimum height=3em, align=center, text width=3em, right=1em of wrapper-1] (wrapper-2) {Block\#2};
    %\node[draw, fill=cyan!30, minimum height=4.5em, yshift=0.6em, right=0em of wrapper-2] (em-2) {\rotatebox{90}{Eviction Manager}};
    \node[minimum height=2em, align=center, right=.5em of wrapper-2] (dots) {...};
    \node[draw, fill=gray!30, minimum height=3em, align=center, text width=3em, right=-0.5em of dots] (wrapper-n) {Block\#3};
    %\node[draw, fill=cyan!30, minimum height=4.5em, yshift=0.6em, right=0em of wrapper-n] (em-n) {\rotatebox{90}{Eviction Manager}};
    
    %\node[draw, fill=white, minimum height=6.5em, right=1.5em of em-n] (ps-api) {\rotatebox{90}{Persistent Store API}};
    \node[draw, fill=yellow!30, minimum height=6.5em, minimum width=3em, right=1.5em of wrapper-n] (ps) {\rotatebox{90}{Persistent Storage}};
    
    %\node[draw, fill=gray!30, minimum height=2em, text width=3.25em, align=center, yshift=-1.25em, at=(wrapper-1.center)] (block-1) {Block\#1};
    %\node[draw, fill=gray!30, minimum height=2em, text width=3.25em, align=center, yshift=-1.25em, at=(wrapper-2.center)] (block-2) {Block\#2};
    %\node[draw, fill=gray!30, minimum height=2em, text width=3.25em, align=center, yshift=-1.25em, at=(wrapper-n.center)] (block-n) {Block\#n};
    
    \begin{pgfonlayer}{bg}
      \draw[gray, very thick, dotted] ($(wrapper-1.south west)+(-0.5em,-1em)$) -- ($(wrapper-n.south east)+(0.5em,-1em)$);
      %\draw[gray, very thick, dotted] ($(wrapper-1.west)+(-12.5em,0.5em)$) -- ($(wrapper-n.east)+(0.5em,0.5em)$);
      %\draw[gray, very thick, dotted] ($(wrapper-1.west)+(-9.5em,4.25em)$) -- ($(wrapper-n.east)+(0.5em,4.25em)$);
      \draw[gray, very thick, dotted] ($(wrapper-1.north west)+(-0.5em,1em)$) -- ($(wrapper-n.north east)+(0.5em,1em)$);
    \end{pgfonlayer}
    
    %\node[fill=white, draw, text width=3.25em, align=center, above=0em of block-1] (block-api-1) {Block API};
    %\node[fill=white, draw, text width=3.25em, align=center, above=0em of block-2] (block-api-2) {Block API};
    %\node[fill=white, draw, text width=3.25em, align=center, above=0em of block-n] (block-api-n) {Block API};
    
    %\node[text width=3.25em, align=center, above=0em of block-api-1] (partition-1) {Block\#1};
    %\node[text width=3.25em, align=center, above=0em of block-api-2] (partition-2) {Block\#2};
    %\node[text width=3.25em, align=center, above=0em of block-api-n] (partition-n) {Block\#n};
    
    %\node[draw, text width=5.5em, align=center, above=0em of wrapper-1] (partition-api-1) {Partition API};
    %\node[draw, text width=5.5em, align=center, above=0em of wrapper-2] (partition-api-2) {Partition API};
    %\node[draw, text width=5.5em, align=center, above=0em of wrapper-n] (partition-api-n) {Partition API};
    
    %\node[draw, fill=white, text width=3.5em, align=center, above=3em of partition-api-1] (client-api-1) {Client API};
    %\node[draw, fill=white, text width=3.5em, align=center, above=3em of partition-api-2] (client-api-2) {Client API};
    %\node[draw, fill=white, text width=3.5em, align=center, above=3em of partition-api-n] (client-api-n) {Client API};
    
    \node[draw, fill=BurntOrange!30, text width=3em, align=center, above=1.5em of wrapper-1] (app-1) {App\#1};
    \node[draw, fill=BurntOrange!30, text width=3em, align=center, above=1.5em of wrapper-2] (app-2) {App\#2};
    \node[draw, fill=BurntOrange!30, text width=3em, align=center, above=1.5em of wrapper-n] (app-n) {App\#m};
    
    \draw[color=black] (app-1.south) -- (wrapper-1.north);
    %\draw[color=black] (client-api-1.south) -- (partition-api-2.north);
    %\draw[color=black] (client-api-1.south) -- (partition-api-n.north);
    
    %\draw[color=black] (client-api-2.south) -- (partition-api-1.north);
    \draw[color=black] (app-2.south) -- (wrapper-2.north);
    %\draw[color=black] (client-api-2.south) -- (partition-api-n.north);
    
    %\draw[color=black] (client-api-n.south) -- (partition-api-1.north);
    %\draw[color=black] (client-api-n.south) -- (partition-api-2.north);
    \draw[color=black] (app-n.south) -- (wrapper-n.north);
    
    \draw[color=black, <->] (wrapper-n.east) -- (ps.west);

    \node[draw, fill=BurntOrange!30, minimum height=2em, text width=3em, align=center, below=1.5em of wrapper-2] (lease) {Lease Manager};
    
    \node[draw, fill=blue!30, minimum height=2em, text width=3em, align=center, left=0.75em of lease] (allocator) {Block Allocator};
    %\node[draw, fill=white, text width=7.5em, align=center, above=0em of allocator] (allocator-api) {Block Allocation API};
    
    
    
    \node[draw, fill=green!30, minimum height=2em, text width=3em, align=center, right=0.75em of lease] (metadata) {Metadata Manager};
    %\node[draw, fill=white, text width=7em, align=center, above=0em of metadata] (metadata-api) {Metadata API};
    
    %\draw[color=black] (allocator.north) -- (wrapper-1.south);
    \draw[color=black] (allocator.north) -- (wrapper-2.south);
    %\draw[color=black] (allocator.north) -- (wrapper-n.south);
    
    %\draw[color=black] (lease.north) -- (wrapper-1.south);
    \draw[color=black] (lease.north) -- (wrapper-2.south);
    %\draw[color=black] (lease.north) -- (wrapper-n.south);
    
    %\draw[color=black] (metadata.north) -- (wrapper-1.south);
    \draw[color=black] (metadata.north) -- (wrapper-2.south);
    %\draw[color=black] (metadata.north) -- (wrapper-n.south);
    
    \node[fill=white, align=center, left=1.4em of allocator] (management) {\rotatebox{90}{{\centering $\strut$Control-plane}}};
    %\node[draw, fill=white, align=center, above=1em of management] (storage) {\rotatebox{90}{{\centering $\strut$Storage}}};
    %\node[draw, fill=white, align=center, above=1.1em of storage] (partition) {\rotatebox{90}{$\strut$Partition}};
    %\node[draw, fill=white, align=center, above=0.5em of partition] (client) {\rotatebox{90}{$\strut$Client}};
    
    %\node[draw, fill=white, text width=1.7em, align=center, left=3.5em of allocator, yshift=2.75em] (system-layer) {\rotatebox{90}{\parbox{11em}{\centering System Layer}}};
    
    \node[fill=white, text width=1.7em, align=center, above=0em of management] (ds-layer) {\rotatebox{90}{\parbox{7.5em}{\centering Data Plane}}};
  \end{tikzpicture}
  \caption{\small\textbf{\sl design details. \rc{Lets convert this figure to a high-level design overview figure; replace ``system layer -- management'' by ``\sl control plane''; combine ``system layer -- storage'' and ``data structure layer -- partition'' to ``\sl data plane''; and remove ``system layer'' and ``data structure layer'' boxes; also remove ``block allocation API (white box)'' and ``metadata API (white box)''; cover ``block allocator'', ``lease manager'' and ``metadata manager'' by ``directory service''}}}
  \label{fig:layer}
\end{figure}
%
}
...
\begin{figure}
  \centering
  \begin{tikzpicture}[font=\scriptsize]
    \node[draw, rounded corners] (l1) {$\lambda$};
    \node[draw, rounded corners, right=0.1em of l1] (l2) {$\lambda$};
    \node[draw, rounded corners, right=0.1em of l2] (l3) {$\lambda$};
    \node[draw, rounded corners, right=2em of l3] (l4) {$\lambda$};
    \node[draw, rounded corners, right=0.1em of l4] (l5) {$\lambda$};
    \node[draw, rounded corners, right=0.1em of l5] (l6) {$\lambda$};
    \node[draw, rounded corners, right=0.1em of l6] (l7) {$\lambda$};
    \node[draw, rounded corners, right=0.1em of l7] (l8) {$\lambda$};
    \node[draw, rounded corners, right=2em of l8] (l9) {$\lambda$};
    \node[draw, rounded corners, right=0.1em of l9] (l10) {$\lambda$};
    \node[draw, rounded corners, right=0.1em of l10] (l11) {$\lambda$};
    \node[draw, rounded corners, right=0.1em of l11] (l12) {$\lambda$};
    \node[draw, rounded corners, right=0.1em of l12] (l13) {$\lambda$};
    
    \node[above=0.1em of l2] {Job\#1};
    \node[above=0.1em of l6] {Job\#2};
    \node[above=0.1em of l11] {Job\#3};
    
    \draw[dashed] ($(l1.south west)+(-1.5em, -.25em)$) -- ($(l13.south east)+(1.5em, -.25em)$);
    
    \node[draw, rounded corners, text width=9em, minimum height=2em, align=center, below=2em of l3] (controller) {\textbf{Controller}: Job-level resource\\ allocation \& scaling};
    \node[draw, rounded corners, text width=7em, minimum height=2em, align=center, right=5em of controller] (metadata) {\textbf{Metadata server(s)}\\Request routing};
    \node[draw, rounded corners, text width=3em, minimum height=2em, align=center, below=0.75em of controller.south west, xshift=1.75em] (s1) {\textbf{Storage}\\\textbf{Server}};
    \node[draw, rounded corners, text width=3em, minimum height=2em, align=center, right=1.1em of s1] (s2) {\textbf{Storage}\\\textbf{Server}};
    \node[draw, rounded corners, text width=3em, minimum height=2em, align=center, right=1.1em of s2] (s3) {\textbf{Storage}\\\textbf{Server}};
    \node[draw, rounded corners, text width=3em, minimum height=2em, align=center, right=1.1em of s3] (s4) {\textbf{Storage}\\\textbf{Server}};
    \node[draw, rounded corners, text width=3em, minimum height=2em, align=center, right=1.1em of s4] (s5) {\textbf{Storage}\\\textbf{Server}};
    
    \draw[blue, thick, dotted, -stealth] (l4.south) to [out=225, in=90] node[left, near end] {\scriptsize \textcircled{1} Register job} (controller.north);
    \draw[blue, thick, dotted, -stealth] (l5.south) to [out=225, in=90] node[right, near end] {\scriptsize \textcircled{3} Deregister job} ($(controller.north)+(1em, 0em)$);
    \draw[blue, thick, dotted, -stealth] (controller.east) -- node[above, text width=4em, align=center] {\scriptsize \textcircled{2} Allocate} node[below, text width=4em, align=center] {resources} (metadata.west);
    
    \node[red, right=0em of l13] (put) {\scriptsize\texttt{put()}};
    \draw[red, thick, dotted, stealth-stealth] (put.south) to node[right, near end] {\textcircled{a}} (metadata.north);
    \draw[red, thick, dotted, -stealth] (put.east) to[bend left=30] node[right, near end] {\textcircled{b}} (s5);
...
\begin{figure}
  \centering
  \begin{tikzpicture}[font=\scriptsize, level distance=1.6em, level 1/.style={sibling distance=3em}]
    \node [draw, draw, fill=gray!40] (a) {\texttt{A}};
    \node [draw, draw, fill=gray!40, right=4em of a] (b) {\texttt{B}};
    \node [draw, rounded corners=0.5em, draw, fill=cyan!40, below=1em of a] (g1) {\texttt{A\_GROUPBY\_x}};
    \node [draw, rounded corners=0.5em, draw, fill=cyan!40, below=1em of b] (g2) {\texttt{B\_GROUPBY\_x}};
    \node [draw, rounded corners=0.5em, draw, fill=cyan!40, below=1em of g1] (g3) {\texttt{P\_GROUPBY\_y}};
    \node [draw, rounded corners=0.5em, draw, fill=cyan!40, below=1em of g3] (j1) {\texttt{R\_JOIN\_Q}};

    \draw [-latex] (a) -- (g1);
    \draw [-latex] (b) -- (g2);
    \draw [-latex] (g1) -- (g3);
    \draw [-latex] (g3) -- (j1);
    \draw [-latex] (g2) -- (j1.east);

    \node [align=center, text width=11em, below=0em of j1, xshift=2em] (lh-1) {\lh for \texttt{Query\#1} (constructed from provided query-plan)};

    \node [draw, fill=gray!40, right=5em of b] (c) {\texttt{C}};
    \node [draw, rounded corners=0.5em, draw, fill=cyan!40, below=1em of c] (g3) {\texttt{C\_GROUPBY\_x}};
    \node [draw, fill=gray!40, right=1em of g3] (d) {\texttt{D}};
    \node [draw, rounded corners=0.5em, draw, fill=cyan!40, below=1.5em of $(g3)!0.5!(d)$] (j2) {\texttt{S\_JOIN\_D}};
    \node [draw, fill=gray!40, right=1em of j2] (e) {\texttt{E}};
    \node [draw, rounded corners=0.5em, draw, fill=cyan!40, below=1.5em of $(j2)!0.5!(e)$] (j3) {\texttt{T\_JOIN\_E}};
    
    \node [align=center, text width=11em, right=0.5em of lh-1] {\lh for \texttt{Query\#2} (deduced based on ephemeral state dependency)};

    \draw [-latex, dashed] (c) -- (g3);
    \draw [-latex, dashed] (g3) -- (j2);
    \draw [-latex, dashed] (d) -- (j2);
    \draw [-latex, dashed] (j2) -- (j3);
    \draw [-latex, dashed] (e) -- (j3);
...
\cut{Storage is allocated to namespaces at the granularity of fixed-capacity memory blocks at the storage service, akin to virtual pages in traditional operating systems. The block-based abstraction admits transparent scaling of storage capacity for each namespace via addition or removal of blocks on workload-driven triggers. Moreover, by employing data structure-specific scaling strategies at namespace granularity, \jiffy ensures scaling allocations for one namespace does not impact other namespaces, thus providing reservation of resources per namespace.

\jiffy logically separates the directory and storage services for a clean separation of responsibilities, and to permit independent scaling for the two services. The directory service conducts management operations on the storage service, including tracking available blocks at each storage server, allocating and removing blocks across different storage servers, detecting and resolving failures across replicated data blocks, etc. In contrast, the storage service handles fine-grained memory allocations at the memory blocks, and serves fine-grained data access and mutation requests across data structure partitions. The storage service also facilitates chain replication at block-granularity.  

The client library connects applications to the two services. While a majority of the client functionality is specific to the data structure implementation, a key responsibility of any client implementation is to fetch and cache crucial metadata such as partition-to-block mappings from the directory service, and use them to perform fine-grained data structure operations directly on the storage service.

As noted earlier, \jiffy namespaces form the basic abstraction for fine-grained elasticity, resource sharing and reservation across different applications. Storage is allocated to namespaces at the granularity of fixed-capacity memory blocks at the storage service, akin to virtual pages in traditional operating systems. The block-based abstraction admits transparent scaling of storage capacity for each namespace via addition or removal of blocks on workload-driven triggers. Moreover, by employing data structure-specific scaling strategies at namespace granularity, \jiffy ensures scaling allocations for one namespace does not impact other namespaces, thus providing reservation of resources per namespace.

There are two main techniques explored in prior work for efficient lifetime management. First, programming languages often employ scoping to determine object lifetimes, reclaiming memory via garbage collection~\cite{javagc, pythongc} for out-of-scope objects. However, the decoupling of ephemeral storage from compute in serverless architectures makes the problem of identifying object scope at the storage layer challenging.
...
%    \draw[-stealth] (t9.east) -- ($(t9.east)+(1.2em, 0)$);
    \draw[-stealth] (t10.east) -- ($(t10.east)+(1.5em, 0)$);
%    \draw[-stealth] (t11.east) -- ($(t11.east)+(1em, 0)$);
  ...
  \end{tikzpicture}
  \vspace{-0.5em}
  \caption{\small\textbf{Execution DAG example for a typical analytics job.} Intermediate data exchange across tasks occurs via \jiffy.}
  \label{fig:example}
  \label{fig:query-example}\vspace{-1em}
\end{figure}
...
%    
  ...
  \end{tikzpicture}
  \vspace{-1em}
  \caption{\textbf{Lease Renewal via Address Hierarchy.} Hierarchical addressing simplifies lease renewal in \jiffy (\S\ref{ssec:mlm}), since lease renewal for an address-prefix automatically implies renewals for all parent and descendent address-prefixes in the hierarchy.}\label{fig:mlm}\vspace{-1em}
\end{figure}
...
%	\renewcommand{\arraystretch}{1.1}
	\footnotesize
	\caption{\small\textbf{\jiffy User-facing API}. See \S\ref{ssec:api} for details.}
	\label{table:api}
	\vspace{0.1in}
	\begin{tabular}{c|l|l}
		\hline
...
\paragraphb{User-facing API} \jiffy's user-facing interface (Table~\ref{table:api}) is divided along its two core abstractions: \textit{hierarchical addresses} and \textit{data structures}. Jobs add a new address-prefix to their \lh using \texttt{createAddrPrefix}, specifying the parent address-prefix, along with optional arguments such as initial capacity. \jiffy also provides a \texttt{createHierarchy} interface to directly generate the complete \lh from the application's execution plan (\ie, DAG), and \texttt{flush}/\texttt{load} interfaces to persist/load address-prefix data from external storage (\eg, S3). \jiffy provides three built-in data structures that can be associated with an address-prefix (via \texttt{initDataStructure}), and a way to define new data structures using its internal API. %\jiffy also allows applications to ship a re-partition function for the data structure (via \texttt{setReartitioner}) allowing \jiffy to conduct repartitioning on its behalf (\S\ref{ssec:fdr}). The \texttt{repartitionFn} takes source and destination blocks as input, and must provide the logic for moving data between the two based on the repartitioning logic. \rc{Reviewer B: What if the application (data structure) writer doesn't know the size of the destination data representation (e.g., if data is compressed)? Anurag's comment: This seems like a very deep corner case -- do we need to address?}
...
\begin{figure}[h]
  ...
  \end{tikzpicture}
  \vspace{-0.5em}
  \caption{\small\textbf{\jiffy controller.} See \S\ref{sec:cp} for details.}
  \label{fig:ds-arch}\vspace{-1em}
\end{figure}
%
...
\paragraphb{Data plane} \jiffy data plane is responsible for two main tasks: providing jobs with efficient, data-structure specific atomic access to data, and repartitioning data across blocks allocated by the control plane during resource scaling. It partitions the resources in a pool of storage servers across fixed sized blocks. Each storage server maintains, for the blocks managed by it, a mapping from unique blockIDs to pointers to raw storage allocated to the blocks, along with two additional metadata: data structure-specific operator implementations as described in \S\ref{ssec:api}, and a subscription map that maps data structure operations to client handles that have subscribed to receive notifications for that operation. %\sl employs this mapping to signal availability of data via a notification service. 

Data repartitioning for a \jiffy data structure is implemented as follows: when a block's usage grows above the high threshold, the block sends a signal to the control plane, which, in turn, allocates a new block to the address-prefix and responds to the overloaded block with its location. The overloaded block then repartitions and moves part of its data to the new block (see Figure~\ref{fig:autoscaling}); a similar mechanism is used when the block's usage falls below the low threshold. %Similarly, when the block usage drops below a low threshold, the block signals the control plane, which responds with the location of another block in the address-prefix with with which it can merge its data using the \texttt{repartition} operator. After repartitioning completes, the block signals the control plane, which then deallocates it from the corresponding address-prefix. The actual data repartitioning is conducted transparently and asynchronously in \jiffy; data access operations across blocks in a data structure can proceed even while repartitioning is in progress.
...
repartitioning when new blocks are added. %\rc{Reviewer B: You assume that files are append-only, which simplifies the elasticity of the file abstraction -- no block removals are needed, and no repartitioning steps are needed when new blocks are added. How would you support non-append updates to your file abstraction?}
...
\paragraphb{Experimental setup} Unless otherwise specified, each intermediate storage system in our experiments is deployed across $10$ m4.16xlarge EC2~\cite{ec2} instances, while serverless applications are deployed across AWS Lambda~\cite{ec2} instances. Since \jiffy builds on Pocket design, it supports addition of new instances to increase the system-wide capacity. However, our experiments do not evaluate overheads for doing so, since, it is orthogonal to \jiffy's goals; we specifically focus on multiplexing available system storage capacity for higher utilization and reducing the need for adding more capacity. \jiffy employs $128$MB blocks, $1$s lease duration and $5\%$ (low) and $95\%$ (high) as thresholds for data repartitioning. %As mentioned above, an analysis of \jiffy's sensitivity to these parameters is presented in Appendix~\ref{ssec:sensitivity}. %\rqc{Finally, our evaluation for Pocket places its control and metadata services on the same server to ensure a fair comparison with \jiffy's unified control plane.} % \rc{Reviewer C: One thing not clear to me is what system architecture is used for Pocket. Does it use separated control and metadata servers? If Honeycomb uses its new combined control+metadata server while Pocket is still separating them, then please evaluate the performance impact of the different settings. I assume combining them would give Honeycomb extra performance advantages since it saves network messages and latency.}
...
\jiffy enables fine-grained resource allocation for serverless analytics. We demonstrate the benefits of this approach to job performance and resource utilization for roughly $50,000$ jobs across $100$ randomly chosen tenants over a randomly selected $5$ hour window in the Snowflake workload\footnote{{We were unable to evaluate the entire $14$ day window with $>2000$ tenants due to intractable cost overheads.}}~\cite{snowset}. %\rc{Reviewer E: Why only show a single 5 hour window in Figure 9? What does the rest of the workload look like? What is the point of showing a single 5 hour window when the paper doesn't even state the total runtime of the job (e.g., was this 80\% of the run or 5\% of the run??)? Why not even say the average or median utilization (or literally any metric of the entire run)?}
%
...
\paragraphb{Impact of fine-grained elasticity on job performance} We demonstrate this impact by constraining the amount of available capacity at the intermediate store for the Snowflake workload. Figure~\ref{fig:perfvcap} shows the average job slowdown as the capacity is reduced to a certain percentage of the peak utilization for the workload within the evaluated time window (\ie, across all jobs). With Elasticache, job performance suffers significantly when the intermediate data grows larger than capacity ($34\times$ slowdown at $20$\% capacity), since the data must now be accessed from S3. With Pocket, the data spills to SSD when the allocated capacity at the DRAM-tier (during job registration) is insufficient. While the slowdown is less severe due to its efficient tiered-storage, jobs still experience a $>4.1\times$ slowdown at $20$\% capacity. Finally, \jiffy observes much lower job performance degradation with constrained capacity ($<2.5\times$ at $20$\% capacity). This is because task-level elasticity and lease based reclamation of faster storage capacity allows \jiffy to efficiently multiplex capacity across multiple jobs at a much finer granularity than Pocket. This, in turn, significantly reduces data spilling over to a slower storage tier in \jiffy compared to Pocket. We confirm this intuition further by studying the impact of fine-grained elasticity on resource utilization next. %\rc{Reviewer A: Figure 9a. What are each of these cases normalized against? Do some of the setups have substantially different performance at 100\% peak? I don't fully understand where the differences between the performance of Pocket and Honeycomb come from here. They both use the same storage tiering. I understand that Honeycomb task-level visibility with respect to managing the capacity which results in lower peak use. But, I don't fully understand how that interacts with the tiering. For example, is part of the benefit here that Honeycomb's leases let it know when earlier release of resources makes sense, which in turn prevents some spilling? Another fuzziness that I had doubts about surrounded the details of Pocket's policy for managing which storage tier data should be at; if data moves between the tiers automatically/spills then that is the eviction/replacement policy? Doesn't that play a role here in the performance that it achieves? Maybe said another way: why is spilling costly in JCT when using Pocket? Do in memory "miss rates" spike when using the same x-axis as in the figure? Somehow I think one more figure with a metric like that comparing Honeycomb to Pocket would have helped, perhaps with a bit more of an explanation why Pocket's (or some other generic eviction policy) doesn't work well here.}
...
We now compare \jiffy performance (using its KV-Store data structure) against five state-of-the-art systems commonly used for intermediate data storage in serverless analytics: S3, DynamoDB, Elasticache, Apache Crail and Pocket. Since only a subset of the compared systems support request pipelining, we disable pipelining for all of them. %We show that \jiffy is able to provide comparable or better performance than these systems, while providing the benefits outlined in \S\ref{ssec:overallbenefits}. %We use the KV-Store data structure for \jiffy in these benchmarks. %Table~\ref{table:requirements} provides a qualitative comparison. % We deploy these systems on m4.16xlarge EC2 instances (64 vCPUs, 256GB RAM and 10Gbps network bandwidth). 
...
\begin{figure}[h]
  \centering
  \subfigure[\scriptsize Streaming word-count] {
    \includegraphics[width = 1.4in]{fig/jiffy/stream_latency}
    \label{fig:stream}
  }%
  \subfigure[\scriptsize Encoding videos on ExCamera] {
    \input{fig/jiffy/excamera_e2e.tikz}
    \label{fig:excamera}
  }\vspace{-1em}
  \caption{{\textbf{Additional applications on \jiffy.}}}
  \label{fig:apps2}\vspace{-1em}
\end{figure}

\subsection{Additional Applications on \jiffy}
\label{ssec:statefulseverlessapps}
\label{ssec:hive}
\label{ssec:sort}
\label{ssec:venc}

The Snowflake workload evaluated in \S\ref{sec:evaluation} shows \jiffy performance a SQL application (MapReduce model, \S\ref{ssec:bsp}). We now evaluate \jiffy for two additional serverless applications, which make use of other models. 

\paragraphb{Streaming word-count} This workload comprises $50$ partition tasks that split input sentences randomly sampled from the Wikipedia dataset~\cite{wikipedia} into words and partition them based on their string hashes, and $50$ count tasks that collect words within a partition and compute their counts. The job employs queues as data channels (Dataflow model) and stores word counts in a KV-store (Piccolo model). We compare \jiffy performance with Elasticache (both hosted on $5$ m4.16xlarge EC2 instances), since both support queue and KV-store data models. Figure~\ref{fig:stream} shows the CDF of end-to-end batch latency (batch size $=64$ sentences). Despite its benefits (\S\ref{ssec:benefits}), \jiffy can match the performance of an over-provisioned Elasticache cluster.

\paragraphb{Video encoding} ExCamera~\cite{excamera} is a video processing framework that facilitates fine-grained parallelism for video encoding on AWS Lambda. It performs encoding using serverless tasks that exchanged state via a dedicated rendezvous server that forwards messages between them. We compare the rendezvous server approach with state exchange via \jiffy queues in ExCamera, both being hosted on a single m4.16xlarge instance. Figure~\ref{fig:excamera} shows ExCamera task latencies for uncompressed 4k raw frames from~\cite{sintel}. Compared to ExCamera, \jiffy reduces task wait times (lighter shade) by $10$-$20$\% via queue notifications.}
