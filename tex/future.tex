...
% \ltp{Whether it is better to place the first sentence in the second paragraph. It seems not affecting the flow even if we remove it or combine with the first sentence of the 2nd paragprah.}
% Key-Value (KV) cache is crucial for optimizing large language model (LLM) serving, especially for meeting service-level objectives (SLOs) like time to the first token (\textbf{TTFT})\cite{slo}. 
...
% This sequential dependency requires the model to perform computation over all preceding tokens, making \llm inference computationally intensive. 
...
% Production systems are often measured by their TTFT, the response time from the start of the prefill stage to the generation of the first token, making the prefill stage a critical performance bottleneck.
...
% However, in production chatbot systems~\cite{?}\Space{ (e.g., ChatGPT, Claude)}, where users often engage in multiple, concurrent chat sessions, new challenges arise. Each session can involve distinct tasks or questions, and when a session is deactivated, its KV cache is discarded and the session data is offloaded to external storage. When the session is reactivated, the entire chat history, along with any new input, must be brought back to the GPU to rebuild the KV cache during the prefill stage. This reloading process demands significant GPU resources and can lead to high latency, which directly affects SLOs like TTFT~\cite{distserve}.
...
% While this can help, it introduces new issues, especially when dealing with large models and extended contexts. 
...
% \ltp{I suggest to remove this sentence or move it later. It gives me the impression that cxl has been proposed before.} \ltp{providing an efficient solution to the challenges posed by KV cache storage in LLMs} 
% By connecting additional DRAM to servers via PCIe, CXL enables systems to extend beyond the memory limits of a single server. 
...
% We present a design a KV cache management system that leverages CXL technology, emphasizing improvements in system efficiency and cost-effectiveness. 
...
% The remainder of this paper is structured as follows: Section 2 discusses related work in memory management for machine learning models. Section 3 describes our proposed approach and its implementation. Section 4 presents our experimental setup and results, followed by a discussion in Section 5. Finally, Section 6 concludes the paper and outlines potential future directions.
...
% \sam{
% \begin{enumerate}
%     \item add cxl to gpt fast
%     \item multibatching to gpt fast
%     \item memory tiering to gpt fast
%     \item experiment setup (hardware)
%     \item measuring interconnect
% \end{enumerate}}
...
\begin{figure*}[ht!]
    ...
    \vspace{-1em}
    \caption{\textbf{\Space{Feasibility of caching KVcache in CXL memory pool.}Experiment results.} (a) Latency and bandwidth measurements across different access sizes, CXL-GPU interconnect performs similarly as CPU-GPU interconnect. (b) TTFT comparison between \kv re-compute and prefix caching with \cxl or GPU. (c) Serving throughput comparison under a fixed \slo constraint (400ms).} 
